...
-----------------------------------------------------------------------------
module Control.Monad.Trans.Either.ExceptionInstance (
...
-- | Hoist
hoistEitherT :: NoExceptionInstance x => (forall b. m b -> n b) -> EitherT x m a -> EitherT x n a
hoistEitherT f =
  EitherT . f . runEitherT
{-# INLINE hoistEitherT #-}
...
type family NoExceptionInstance e :: Constraint
type instance NoExceptionInstance e =
  TypeError
    ( 'Text "Error: Type '" ':<>: 'ShowType e
      ':<>: 'Text "' has an exception instance"
      ':$$: 'Text "and hence should not be used with EitherT."
      )
